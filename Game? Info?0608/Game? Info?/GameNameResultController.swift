...
class GameNameResultController: UITableViewController, XMLParserDelegate{
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        if segue.identifier == "segueToLastDetail"{
        if let cell = sender as? UITableViewCell{
            let indexPath = tableView.indexPath(for: cell)
            gametitle = (posts.object(at: (indexPath?.row)!) as AnyObject).value(forKey: "gametitle") as! NSString as String
            ...
            if let lastDetailController = segue.destination as?
                ...
                lastDetailController.url = url! + "gametitle=" + gametitle
            }
        }
    }
    }
